...
OSStatus kcCreate(const char *path, const char *password, int pwLen) {
  ...
  OSStatus err =
      SecKeychainCreate(path, (UInt32)pwLen, password, (Boolean)0, NULL, &ref);
  if (ref)
    CFRelease(ref);
  return err;
}

/* kcDelete removes the keychain at path from the search list and deletes the
 * file. SecKeychainDelete does not free the ref — CFRelease must be called
 * separately. */
...
int kcGet(const char *path, const char *service, const char *account,
  ...
  *outErr = noErr;
   ...
   * dialog on the SecItemCopyMatching call below instead. */
  ...
  const void *vals[] = {
      kSecClassGenericPassword, svc,           acc, searchList,
      kSecMatchLimitOne,        kCFBooleanTrue};
  CFDictionaryRef query =
      CFDictionaryCreate(NULL, keys, vals, 6, &kCFTypeDictionaryKeyCallBacks,
                         &kCFTypeDictionaryValueCallBacks);
  CFTypeRef result = NULL;
  OSStatus err = SecItemCopyMatching(query, &result);
  CFRelease(query);
 ...
 * Uses persistent refs to ensure only items from the target keychain are
 * deleted. Returns 0 on success (including when there are no items), -1 on
 * error (sets *outErr). */
int kcClearItems(const char *path, OSStatus *outErr) {
  *outErr = noErr;
  ...
  const void *qvals[] = {kSecClassGenericPassword, desc, searchList,
                         kSecMatchLimitAll, kCFBooleanTrue};
  CFDictionaryRef query =
      CFDictionaryCreate(NULL, qkeys, qvals, 5, &kCFTypeDictionaryKeyCallBacks,
                         &kCFTypeDictionaryValueCallBacks);
  CFTypeRef result = NULL;
  OSStatus err = SecItemCopyMatching(query, &result);
  CFRelease(query);
...
OSStatus kcAdd(SecKeychainRef kref, const char *service, const char *account,
  ...
  CFStringRef acc =
      CFStringCreateWithCString(NULL, account, kCFStringEncodingUTF8);
  CFStringRef desc =
      CFStringCreateWithCString(NULL, description, kCFStringEncodingUTF8);
  CFStringRef lbl =
  ...
  CFArrayRef trustedList =
      CFArrayCreate(NULL, (const void **)&selfApp, 1, &kCFTypeArrayCallBacks);
  ...
  CFDictionaryRef attrs =
      CFDictionaryCreate(NULL, keys, vals, 8, &kCFTypeDictionaryKeyCallBacks,
                         &kCFTypeDictionaryValueCallBacks);
  ...
  if (err == errSecDuplicateItem) {
    CFArrayRef searchList =
        CFArrayCreate(NULL, (const void **)&kref, 1, &kCFTypeArrayCallBacks);
    ...
    CFDictionaryRef query = CFDictionaryCreate(
        NULL, qkeys, qvals, 4, &kCFTypeDictionaryKeyCallBacks,
        &kCFTypeDictionaryValueCallBacks);
    /* kSecAttrAccess is intentionally omitted from the update dict — including
     * it overwrites the existing access control and triggers multiple
  ...
  *outDates = NULL;
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (svc == NULL) {
      ...
      CFIndex len = CFStringGetMaximumSizeForEncoding(CFStringGetLength(svc),
    ...
    if (modDate == NULL) {
      ...
      /* CFAbsoluteTime: seconds since 2001-01-01 UTC; Unix epoch offset =
       * 978307200 */
